...
const Day = ({ data }) => {
  ...
  const filterNextSevenDaysData = (weatherData) => {
    ...
    weatherData.forEach(dataPoint => {
      const date = moment(dataPoint.dt_txt, 'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD');
      if (!groupedData[date]) {
        groupedData[date] = [];
      }
    ...
    const nextSevenDaysData = Object.values(groupedData)
      ...
      .map(dataPoints => dataPoints[0]); // You might need to adjust this based on your data structure
      ...
      <div className="text-center flex justify-center items-center">
        {nextSevenDaysData.slice(0, 5).map((item, index) => (
          ...
          >
            ...
            <Temperature className="pl-1">{`${parseFloat(
              item.main.temp
